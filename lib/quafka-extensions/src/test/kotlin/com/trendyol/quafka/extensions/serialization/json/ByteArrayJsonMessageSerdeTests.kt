...
class ByteArrayJsonMessageSerdeTests :
    FunSpec({
        val typeResolver = mockk<TypeResolver>(relaxed = true)
        val serde = ByteArrayJsonMessageSerde(objectMapper, typeResolver)
        ...
        test("should serialize key") {
            ...
            // Assert
            result shouldBe "12345".toByteArray(HeaderParsers.defaultCharset)
        }
        ...
        test("should serialize value") {
            // Arrange
            val value = DummyObject(id = "12345")

            // Act
            val result = serde.serializeValue(value)

            // Assert
            result shouldBe "{\"id\":\"12345\"}".toByteArray(HeaderParsers.defaultCharset)
        }
        ...
        test("should deserialize key") {
            // Arrange
            val incomingMessage = mockk<IncomingMessage<ByteArray?, ByteArray?>>(relaxed = true)
            every { incomingMessage.key } returns "testKey".toByteArray(HeaderParsers.defaultCharset)
